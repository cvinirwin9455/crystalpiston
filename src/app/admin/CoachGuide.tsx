...
const guideSections: GuideSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "Edit their personal details: name, email, gender, birthday",
      "Manage their training plans (see 'Creating a Client Plan' below for details)",
      "Track payment status with the visual progress bar",
      "Archive clients when their plan ends (they lose dashboard access but all data is preserved)",
    ...
    tips: [
      ...
      "You must create a Plan before you can build any training weeks (see next section)",
    ],
    content: `The Account tab for each client is where you manage the business side: their personal details, training plans, payment tracking, and status. **Important:** You must create an active Plan here before you can build training weeks for a client.`,
  },
  {
    id: "client-plan",
    title: "Creating a Client Plan (Required Before Programming)",
    category: "Client Management",
    icon: "calendar-plus",
    keywords: ["plan", "create plan", "active plan", "required", "start date", "end date", "goal", "race", "no active plan", "before programming"],
    steps: [
      "Select a client → 'Account' tab → click '+ New Plan'",
      "Fill in the required fields: Goal, Start Date, End Date",
      "Optionally set: Target Distance (5K, 10K, Half, Marathon, Ultra, No Race), Race Date, Goal Pace, Plan Cost, Injuries/Notes",
      "Optionally assign a Training Program (see 'Using Program Templates' below)",
      "Click 'Create Plan' — the plan is now active",
      "You can now go to 'Create Week' to start building training weeks within this plan's date range",
    ],
    tips: [
      "A client can only have ONE active plan at a time — you must 'Mark Complete' on the current plan before creating a new one",
      "All training weeks must fall within the plan's start and end dates. If you try to create a week outside those dates, you'll get a warning and won't be able to save",
      "If a client needs a longer plan, edit the plan's end date in the Account tab rather than creating a new plan",
      "The Race Date is used by Program Templates to calculate which program week to auto-load",
      "Completed plans are kept in a collapsible 'Show completed plans' section for history",
    ],
    content: `**A plan is required before you can program any training weeks.** If you go to 'Create Week' without an active plan, you'll see a yellow warning: 'No Active Plan — You need to create an active plan before you can build weekly training.' A plan defines the training period (start & end dates), the client's goal, race details, payment terms, and optionally links to a multi-week program template. Weeks must fall within the plan dates.`,
  },
  {
    id: "program-templates",
    title: "Using Program Templates (Multi-Week Plans)",
    ...
    steps: [
      ...
      "Now when you go to 'Create Week' and select a date, the system automatically calculates: 'Race is X weeks away → load Program Week Y'",
      "The Create Week form auto-fills with that program week's workouts (focus, all 7 days, types, distances, coach notes)",
      "Review the auto-populated plan, make any client-specific tweaks, then Save/Publish as normal",
    ],
    tips: [
      "Programs are assigned to the PLAN, not directly to individual weeks — this is the key concept",
      "The program auto-fills the form only when the form is blank (it won't overwrite edits you've already made)",
      "A purple banner in the Create tab shows: program name, race date, weeks out, and which program week is auto-loading",
      "You can still modify everything after auto-fill — it's a starting point, not locked in",
      "If the coach changes the date range picker to a different week, the system recalculates and loads the correct program week",
      "Distance units are automatically converted if your preference differs from how the program was built",
      "Programs are org-wide — all coaches on your team can assign them to any client's plan",
    ],
    content: `Program Templates are multi-week structured training plans (e.g., '20-Week Marathon Program'). They work by being **assigned to a client's Plan** — not loaded into individual weeks manually. Once assigned, the system automatically calculates which program week to load based on the Race Date and the selected week's start date. When you open Create Week and pick a date, the form auto-fills with the correct program week. This makes it effortless to follow a structured progression while still allowing per-client customization.`,
  },
  {
    id: "assign-coaches",
  ...
  // === PROGRAMMING ===
  {
    ...
    category: "Programming",
    icon: "calendar-plus",
    keywords: ["create", "week", "plan", "program", "build", "training", "schedule", "date range", "no active plan"],
    steps: [
      "Select a client → click 'Create Week' tab",
      "If you see a yellow 'No Active Plan' warning — go to Account tab and create a plan first (weeks cannot be created without one)",
      "A green banner shows your active plan details (goal, dates, payment status)",
      "If a program template is assigned, a purple banner shows which program week will auto-load",
      "Click the date picker and select a Monday-Sunday week (the calendar shows green/yellow dots for existing weeks)",
      "If the week falls outside plan dates, a red warning appears and you can't save — update plan dates or pick a different week",
      "If a week already exists for that date range, it warns you (no duplicates allowed)",
      "Set Week Focus (e.g. 'Speed & Long Run') and Coach Message (shown at top of client's view)",
      "Build each day: select type, subtype, distance, pace, title, description, coach notes",
      "Run/Walk workouts require a subtype AND distance — the form highlights these in orange if missing",
      "Click 'Save as Draft' to review later, or 'Save & Publish' to make it visible immediately",
    ],
    tips: [
      "The date picker shows visual indicators: green dot = published, yellow dot = draft, yellow border = current week",
      "If a program template is assigned, the form auto-fills when you pick a date — just review and tweak",
      "You can also 'Load Template' from your saved week templates (searchable dropdown at the top of the form)",
      "Coach Notes appear highlighted in gold on the client's card — use them for important cues",
      "You can add multiple workouts per day with the '+ Add Workout' button",
      "The weekly mileage total is calculated automatically at the bottom of the form",
      "After publishing, the view jumps to that week so you can verify how it looks",
    ],
    content: `Building a training week is the core of your coaching workflow. **Important prerequisites:** the client must have an active Plan, and the week must fall within that plan's date range. The system enforces this — you'll get clear warnings if something doesn't line up. Once those are set, each week has 7 days (Mon–Sun), and each day can have multiple workouts with full details.`,
  },
  {
  ...
  },
  {
    ...
    category: "Programming",
    icon: "edit",
    keywords: ["draft", "save", "unpublished", "review", "edit", "update", "publish", "unpublish"],
    steps: [
      "When building a week, click 'Save as Draft' — the week is saved but hidden from the client",
      "Drafts appear in the 'Drafts' tab for that client with a yellow 'Draft' badge",
      "Each draft shows: date range, focus, workout count, total mileage, and a 7-day grid preview",
      "Click 'Edit' on a draft to load it back into the Create Week form for changes",
      "Click 'Publish' to make it visible to the client immediately",
      "Click 'Delete' to permanently remove a draft (with confirmation)",
      "You can also unpublish a published week (Training & Logs → moves it back to drafts)",
      "Your dashboard shows total unpublished drafts across all clients as a reminder",
    ],
    tips: [
      "Use drafts to prepare weeks in advance (e.g., program 2–3 weeks ahead on a Sunday)",
      "The dashboard 'Drafts to Publish' widget helps you never forget to publish pending weeks",
      "You can't create a duplicate week — if a published week exists for a date range, you must edit it or unpublish first",
      "Editing a draft replaces the old version (it deletes and recreates) — your changes are saved fresh",
      "Drafts still must fall within the plan's date range — the same validation applies",
    ],
    content: `Drafts let you work on training plans without the client seeing them. The workflow is: Create → Save as Draft → Review → Publish when ready. This is perfect for batch-programming multiple weeks in advance. Drafts are private to coaches — clients only ever see published weeks.`,
  },
  {
    id: "edit-published",
    ...
    category: "Programming",
    icon: "pencil",
    keywords: ["edit", "change", "published", "update", "modify", "adjust", "live", "locked", "past"],
    steps: [
      "Select the client → go to 'Training & Logs' tab",
      "Navigate to the published week you want to edit (use left/right arrows)",
      "Click the 'Edit Week' button (only available for current and future weeks)",
      "Make your changes to workouts: type, subtype, distance, pace, descriptions, coach notes",
      "You can also edit the Weekly Coach Message",
      "Click 'Save Changes' — updates appear immediately for the client",
      "Past weeks are locked and show 'Past week (locked)' — they cannot be edited",
    ],
    tips: [
      "If a client has already logged a workout, their log data is preserved when you edit",
      "To make bigger changes (add/remove days), unpublish the week to drafts, edit there, and republish",
      "You can unpublish a week from Training & Logs — it moves back to drafts and disappears from the client's view",
      "Distance must be a number with up to 2 decimal places (e.g. 4.34) — you'll get an error otherwise",
      "Use this to adjust plans mid-week based on client feedback or injury updates",
    ],
    content: `Plans change — injuries happen, life gets busy, or a client crushes a workout and you want to ramp up. You can edit any published week for the current or future weeks. Changes are reflected immediately on the client's dashboard. Past weeks are locked to preserve training history integrity.`,
  },
  {
    id: "templates",
    title: "Using Week & Day Templates",
    category: "Programming",
    icon: "copy",
    keywords: ["template", "duplicate", "copy", "reuse", "save template", "base plan", "load template", "week template", "day template"],
    steps: [
      "SAVING: After building a week in the Create tab, click 'Save as Template' at the bottom",
      "Give it a name (e.g. 'Base Building Week 3') and optional category (e.g. 'Base Building')",
      "The entire week (focus, coach message, all 7 days) is saved as a reusable template",
      "SAVING DAYS: Each day also has a save-as-template icon to save individual day workouts",
      "LOADING: When creating a new week, use the 'Load Template' dropdown at the top of the Create form",
    ...
    tips: [
      ...
      "Great for recurring patterns: 'Easy Recovery Week', 'Marathon Taper', 'Base Building Block 1'",
      "Loading a week template overwrites the current form content (focus, message, and all days)",
      "Loading a day template into a day that already has content adds it as an additional workout",
      "Templates are different from Program Templates — templates are loaded manually, programs auto-load based on race date",
      "You can also create templates from scratch in the Templates view (without going through Create Week first)",
      "Week templates show a preview: number of runs, cross-training days, and rest days",
    ],
